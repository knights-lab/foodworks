# Modfied with permission from https://github.com/knights-lab/dietstudy_analyses/blob/master/lib/results_scripts/3_Food%20choices%20associate%20with%20microbiome%20composition/1_Figure2C_H/Figure2C_2H.R 
...
setwd("/mnt/nvidia/pkr/code/foodworks")
...
tax <- read.table(file="data/unweighted.mean.unifrac.txt", header=T)

# drop soylents
tax <- tax[,colnames(tax) %in% colnames(food_un)]

tax_all_un <- read.delim("data/unweighted.unifrac.txt", row = 1) # weighted in not significant
# tax_dist <- as.dist(tax_un)

tax_all_dist <- as.dist(tax_all_un)
...
source(file = "scripts/UserNameColors.R")

# tax_all <- read.delim("data/taxonomy_clr_s.GTDB98.txt", row = 1)
...
#   geom_point(aes(color = UserName), alpha=0.75, size = 2, alpha = 0.6) +
...
trans_pro$type <- "Microbiome (Unweighted Unifrac)"
...
taxa_beta <- read.table(file="data/unweighted.mean.unifrac.txt", header=T)
...
save_plot("./figures/Figure2C_H.pdf", plotC_H, base_width = 3.5, base_height = 5.5)


plotC_H_leg <- plot_grid(food_micro_leg, nutr_micro_leg, grain_leg, fruit_leg, ncol = 1, align = "h")

save_plot("./figures/Figure2C_H_legend.pdf", plotC_H_leg, base_width = 3, base_height = 2)