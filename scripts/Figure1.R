# Modfied with permission from https://github.com/knights-lab/dietstudy_analyses 
...
microbes <- read.delim("data/UN_tax_mean_norm_s.GTDB98.txt", row = 1)
...
taxa <- read.table("data/taxonomy_s.GTDB98.txt", header = T, row.names = 1, sep = "\t", comment = "")
...
colors_micro <- sample(c("lightgrey", "#afd35a", "#ec9bfa", "#3c3e00", "#aaa4e1", "#f4f4f4", "#a717d9","#f4e909",
                ...
                "#01ceff", "#02531d", "#d7dd4c", "#8f0000", "#4b03a9", "#A0447F"), no_cols, replace=T)
...
modules <- read.table("data/kegg.txt", header = T, row.names = 1, sep = "\t", comment = "")
...
save_plot("figures/figure1.pdf", bigplot, base_width = 7.9, base_height = 8.26 )
...
save_plot("figures/figure1_legend.pdf", bigplotlegend, base_width = 13.25, base_height = 5)
